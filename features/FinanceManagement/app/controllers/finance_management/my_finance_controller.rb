...
module FinanceManagement
  class MyFinanceController < ApplicationController
    ...
    def index
      ...
      @customer_consume = UserDefinedConsumeType.where(user_id:current_user_id)

      @my_finance = CustomerConsume.where(user_id:current_user_id).reverse.paginate(:page => params[:page], :per_page => 10)
